    ...
    <el-main>
      <el-row :gutter="10">
        ...
        >
          ...
          <h1 @click="toSingerDetailPage(item.id)" style="cursor:pointer;">
            {{ item.name }}
          </h1>
        </el-col>
      </el-row>
...
export default {
  ...
  methods: {
    ...
    changeMusicListSex(typeId) {
      //撤销上一次被选中的标签的样式
      this.$(".isActiveByTypes")
        .eq(0)
        .attr("class", this.prevClickType);
      //这次点击的标签高亮
      this.$(".type" + typeId)
        .eq(0)
        .attr("class", "isActiveByTypes");
      //赋值存储
