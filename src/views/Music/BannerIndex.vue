  ...
  <div>
    <el-container style="margin-top: 25px;margin-bottom: 45px;">
      <el-header height="300px;">
        ...
        <el-carousel type="card" height="300px;">
          <el-carousel-item v-for="(item, index) in bannerInfo" :key="index">
            <el-image @click="changeUrl(item.targetId)" :src="item.imageUrl" fit="contain" style="height: 300px;"></el-image>
          </el-carousel-item>
        </el-carousel>
      </el-header>
      ...
      <el-main>
        ...
        <el-row class="haha" :gutter="12">
          <el-col :span="4" v-for="(item, index) in musicList" style="margin-top: 20px;position:relative;" :key="index">
            <!--播放量-->
            ...
            <div class="songimg">
              <el-image :src="item.coverImgUrl" @click="toSongListPage(item.id)" style="box-shadow: 0 0 2px 2px gray;border-radius: 10px;cursor: pointer">
              </el-image>
            </div>

            <h1 id="songtitle" @click="toSongListPage(item.id)" style="cursor:pointer;">
              {{ item.name }}
            </h1>
          </el-col>
        </el-row>
        ...
        <el-row :gutter="40" style="margin-top: 25px;">
          <el-col :span="8" v-for="item in privateList" :key="item.id">
            <i class="el-icon-video-play" style="color: rgba(255,255,255,0.4);font-size: 45px;position: absolute;top: 10px;z-index: 10"></i>
            <el-image @click="toVideoPage(item.id)" :src="item.sPicUrl" style="box-shadow: 0 0 2px 2px gray;border-radius: 5px;cursor:pointer;"></el-image>
            <h1 id="songtitle" @click="toVideoPage(item.id)" style="cursor: pointer">
              {{ item.name }}
            </h1>
          </el-col>
        </el-row>
        ...
        <el-row style="margin-top: 25px;">
          <!--左侧-->
          <el-col :span="12">
            <el-table :data="newMusicList.slice(0, 5)" stripe @row-dblclick="dblclickPlayMusic" :show-header="false">
              ...
              <el-table-column>
                <template scope="scope">
                  <i class="el-icon-video-play" @click="changeUrl(scope.row.id)" style="position: absolute;font-size: 25px;top: 35px;left:35px;z-index: 10;cursor:pointer;"></i>

                  <el-image :src="scope.row.picUrl" @click="changeUrl(scope.row.id)" style="width: 75px;height: 75px;cursor:pointer;"></el-image>
                  <span style="position: absolute;top: 25px;font-size: 15px;font-weight: 500">
                    {{ scope.row.name }}
                  </span>
                  <span @click="toSingerPage(scope.row)" style="cursor:pointer;position: absolute;bottom: 25px;font-size: 12px;font-weight: 500">
          ...
          <!--右侧-->
          <el-col :span="12">
            <el-table :data="newMusicList.slice(5)" @row-dblclick="dblclickPlayMusic" stripe :show-header="false">
              <el-table-column type="index">
                <template scope="scope">
                  {{
                    scope.$index >= 4
