<template>
  <div style="margin-bottom: 55px;">
    ...
    <div v-if="musicRankList[0].coverImgUrl">
      <el-row style="">
        <el-col :span="6" :offset="1" v-for="(item, index) in 5" :key="index" style="margin-top: 25px;">
          <el-image :src="musicRankList[item - 1].coverImgUrl" @click="toSongListPage(musicRankList[item - 1].id)" style="height: 200px;border-radius: 15px;cursor:pointer;"></el-image>
          <!-- 下表格   @row-dblclick="playMusicList"-->
          <el-table row-class-name="row-height" :data="musicRankList[item - 1].tracks" @click="toSongListPage(musicRankList[item - 1].id)" :show-header="false">
    ...
    <el-row :gutter="10">
      <!-- 从5开始截 是因为上面有6个了 -->
      <el-col :span="4" v-for="(item, index) in musicRankList.slice(5)" style="padding: 10px;position:relative;" :key="index">
        <!--播放量-->
        ...
        <h1 @click="toSongListPage(item.id)" style="cursor:pointer;
